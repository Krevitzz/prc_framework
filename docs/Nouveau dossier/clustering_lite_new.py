"""
analysing/clustering_lite_new.py
...
def prepare_matrix(
    ...
    feat_mask  = [not f.startswith('health_') for f in data.feat_names]
    feat_names = [f for f, keep in zip(data.feat_names, feat_mask) if keep]
    M_raw      = data.M[:, feat_mask].astype(np.float64)
...
def run_clustering(
    ...
    Args:
        ...
        n          : nombre total de runs
        peeling_cfg: cfg['peeling'] depuis analysing_default.yaml
        M_2d       : t-SNE optionnel pour visualisations
