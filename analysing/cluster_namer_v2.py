...
def load_namer_config(path) -> Dict:
    """Charge config namer depuis YAML via load_yaml de data_loading_new."""
    return load_yaml(Path(path))['namer']
...
class ClusterNamer:
    ...
    Usage :
        namer = ClusterNamer.from_yaml()  # chemin par défaut
    ...
    @classmethod
    def from_yaml(cls, path=None) -> 'ClusterNamer':
        if path is None:
            path = Path(__file__).parent / 'configs' / 'cluster_namer.yaml'
        return cls(load_namer_config(path))
