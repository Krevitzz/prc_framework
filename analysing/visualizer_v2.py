...
from utils.data_loading_new import fix_str
    ...
    def plot_signature_heatmap(self, output_dir: str, label: str):
        ...
        SLOT_LEGENDS = {
            'LAG' : (
                ...
                '  valeur faible (ex: LAG-)  → dynamique rapide, décorrélation immédiate\n'
                '  valeur élevée (ex: LAG+)  → persistance longue, évolution lente'
            ),
            'PNN' : (
                'PNN = proportion grandes variations dans signal entropie  [0 – 1]\n'
                '  PNN-  → signal entropie parfaitement régulier (lisse)\n'
                '  PNN+  → variations brusques fréquentes (chaotique)'
            ),
        }
