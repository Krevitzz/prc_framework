        ...
        for chunk in generate_chunks(run_config, registries, chunk_size):
            ...
            if _check_invalid(chunk, registries['gamma']):
                ...
                try:
                    ...
                    signals_b, last_states = run_batch_fn(
                        chunk['gamma_fn'], gamma_params_b,
                        chunk['mod_fn'],  mod_params_b,
                        D_batch, keys_batch, chunk['max_it'],
                    )
                    last_states.block_until_ready()

                    rows = _rows_from_chunk(chunk, chunk, signals_b, last_states)
                    del D_batch, signals_b, last_states
                    ...
                    rows = _fallback_individual(chunk, chunk, registries)
                    for r in rows:
                        s = r['run_status']
                        if s == 'OK':          n_ok        += 1
