...
def detect_outliers(
    ...
    M_ml, feat_names_ml = data.features_for_ml()
    M_clean             = _sanitize_for_isolation_forest(M_ml)

    n_nan = int(np.sum(np.isnan(M_ml)))
    if n_nan > 0:
    ...
    print(f"  Features utilisées : {M_clean.shape[1]}")
...
def analyze_outliers(data: AnalysingData, contamination: float = 0.1) -> Dict:
    ...
    _, feat_names_ml = data.features_for_ml()
    ...
    return {
        ...
        'n_features_used': len(feat_names_ml),
    }
